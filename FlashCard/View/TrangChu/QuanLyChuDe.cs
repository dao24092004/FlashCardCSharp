...
{
    public partial class QuanLyChuDe : UserControl
    {
        ...
        public QuanLyChuDe()
        {
            ...
            InitializeComponent();
            this.user = Form1.LoggedInUser;
            ServiceTopic = new TopicService();
        ...
        private void InitializeGridView()
        {
            ...
            dataGridView1.ColumnCount = 3; // Số lượng cột chính (không tính icon)
            dataGridView1.Columns[0].Name = "ID";
            dataGridView1.Columns[1].Name = "Chủ đề ";
            dataGridView1.Columns[2].Name = "Số lượng từ";
        ...
        private void LoadData()
        {
            try
            {
                ...
                HashSet<object[]> list = ServiceTopic.GetAll(user.UserID);
                ...
                {
                    dataGridView1.Rows.Add(item[0], item[1], item[2]); // vocab_id, word, meaning, topic_name.
                }
            }
        ...
        private void DataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0) // Đảm bảo không nhấn vào header
            {
                if (dataGridView1.Columns[e.ColumnIndex].Name == "EditIcon")
                {
                    ...
                    using (AddTopicForm addForm = new AddTopicForm(user.UserID, id))//0 o day la turong hop add
                    {
                ...
                {
                    int id = int.Parse(dataGridView1.Rows[e.RowIndex].Cells["ID"].Value.ToString());
                    Topic topic = ServiceTopic.FindById(id);
                    var confirmResult = MessageBox.Show($"Bạn có chắc muốn xóa từ  {topic.TopicName}?", "Xác nhận", MessageBoxButtons.YesNo);
                    if (confirmResult == DialogResult.Yes)
                    {
                        ServiceTopic.DeleteById(id);
                        LoadData(); 
                    }
                }
            }
        ...
        {
            // Tạo instance của AddWordForm
            using (AddTopicForm addForm = new AddTopicForm(user.UserID,0))//0 o day la turong hop add
            {
                // Hiển thị AddWordForm dưới dạng hộp thoại
                if (addForm.ShowDialog() == DialogResult.OK)
                {
